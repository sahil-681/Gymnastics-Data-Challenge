...
old_run_sims <- function(competitors, qual36, long_meanstds, gender, my_apps, do_sampling=F){
  ...
  alt_quals <- merge(alt_quals, data.frame(App = my_apps), by=NULL)
  ...
  aa_df$Total_Score <- rowSums(aa_df[, my_apps])
  ...
  qual_aa <- merge(qual_aa, data.frame(App=my_apps), by=NULL)
